...
def normalize_profile(name, profile, defaults):
    ...
    try:
        normalized["percent"] = clamp(int(profile.get("percent", defaults["percent"])), 0, 100)
    ...
    if not isinstance(cfg["profiles"], dict):
        ...
        for name, profile in cfg["profiles"].items():
            if not isinstance(name, str):
                continue
            normalized = normalize_profile(name, profile, defaults)
...
def apply_profile(cfg, dev, profile, save=True):
    color = profile["color"]
...
def cmd_profile_load(cfg, dev, name):
    name = normalize_profile_name(name)
    profiles = get_profiles(cfg)
    ...
    apply_profile(cfg, dev, profiles[name], save=True)
...
""")
    ...
    if cmd == "status":
        ...
        if subcmd == "save":
            ...
            dev = find_device()
            cmd_profile_load(cfg, dev, args[2])
        elif subcmd == "list":
