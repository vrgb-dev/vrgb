...
Show detected device and saved configuration
...
Adjust brightness while keeping the same color.
...
Soft-off the keyboard LEDs.

vrgb off
Restore Saved State

Restore saved color and brightness.
...
Display device detection and HID report details.

vrgb --debug status

Useful when testing compatibility with new hardware.

About

Display project info and ASCII banner.
...
Group membership must refresh before non-root access works.

Optional KDE Autostart Restore

Some ASUS firmware resets the keyboard color at boot.

To restore your saved state automatically, create:
...
Hardware testing on additional ASUS laptops is welcome.

If your device exposes a HID LampArray keyboard interface, please open an issue with:
